export async function ensureMasterTables(pool) {
  const query = async (sql, params = []) => {
    const [rows] = await pool.execute(sql, params);
    return rows;
  };

  // 1. master_users_registry
  await query(`CREATE TABLE IF NOT EXISTS master_users_registry (
  ...
  // 2. saas_user_profiles
  await query(`CREATE TABLE IF NOT EXISTS saas_user_profiles (
    email VARCHAR(255) PRIMARY KEY,
  ...
  // 3. saas_app_settings
  await query(`CREATE TABLE IF NOT EXISTS saas_app_settings (
  ...
  // 5. saas_payments
  await query(`CREATE TABLE IF NOT EXISTS saas_payments (
  ...
  // 6. saas_login_activity
  await query(`CREATE TABLE IF NOT EXISTS saas_login_activity (
    id INT AUTO_INCREMENT PRIMARY KEY,
    email VARCHAR(255) NOT NULL,
  ...
  // 7. saas_license_keys
  await query(`CREATE TABLE IF NOT EXISTS saas_license_keys (
    license_key VARCHAR(100) PRIMARY KEY,
    plan_id VARCHAR(60) NOT NULL,
  ...
  try {
    await query('ALTER TABLE saas_license_keys ADD COLUMN max_users INT DEFAULT 1');
  } catch (err) {
    // Safe to ignore if column is already present
  }

  // 8. saas_contact_messages
  await query(`CREATE TABLE IF NOT EXISTS saas_contact_messages (
    id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    email VARCHAR(255) NOT NULL,
    subject VARCHAR(255),
  ...
  // 9. saas_error_logs
  await query(`CREATE TABLE IF NOT EXISTS saas_error_logs (
    id INT AUTO_INCREMENT PRIMARY KEY,
  ...
  // 11. businesses (V3 Routing Table)
  ...
  try {
    await query('ALTER TABLE businesses ADD COLUMN is_active BOOLEAN DEFAULT TRUE');
  } catch (err) {
    // Safe to ignore if column is already present
  }

  // 12. business_users (V3 Routing Table)
  await query(`CREATE TABLE IF NOT EXISTS business_users (
  ...
  try {
    await query('ALTER TABLE business_users ADD COLUMN invited_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP');
  } catch (err) {
    // Safe to ignore if already present
  }

  try {
    await query('ALTER TABLE business_users CHANGE COLUMN id business_user_id BIGINT AUTO_INCREMENT');
  } catch (err) {
    // Safe to ignore if column is already business_user_id or if CHANGE is not applicable
  }
}
